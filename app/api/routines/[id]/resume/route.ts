...
import { activateSchedule } from "@/lib/routines/trigger";
...
export async function POST(
  ...
  if (routine.triggerDevTaskId) {
    try {
      await activateSchedule(routine.triggerDevTaskId);
    } catch (err) {
      console.error("Failed to activate Trigger.dev schedule:", err);
    }
  }
